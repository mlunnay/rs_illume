use std::sync::Arc;
use std::fmt;
use super::pbrt::{Float, Spectrum, consts::{INV_4_PI, PI}};
use super::geometry::{Ray, Vector3f, Point2f, coordinate_system, spherical_direction};
use super::sampler::Sampler;
use super::interaction::MediumInteraction;
use super::profiler::Profiler;
use obstack::Obstack;

pub trait Medium: fmt::Debug {
    fn tr(&self, ray: &Ray, sampler: Box<dyn Sampler>) -> Spectrum;
...
pub trait PhaseFunction: fmt::Display {
    fn p(&self, wo: &Vector3f, wi: &Vector3f) -> Float;

    fn sample_p(&self, wo: &Vector3f, wi: &Vector3f, u: &Point2f) -> Float;
}
...
impl PhaseFunction for HenyeyGreenstein {
    fn p(&self, wo: &Vector3f, wi: &Vector3f) -> Float {
        let _profile = Profiler::instance().profile("PhaseFunc HenyeyGreenstein evauluation");
    ...
    fn sample_p(&self, wo: &Vector3f, wi: &Vector3f, u: &Point2f) -> Float {
        ...
        let cos_theta: Float = if self.g.abs() < 1e-3 {
            1.0 - 2.0 * u[0];
        } else {
        ...
        let sinTheta = (1.0 - cos_theta * cos_theta).max(0.0).sqrt();
        let phi = 2.0 * PI * u[1];
        let v1 = Vector3f::default();
        let v2 = Vector3f::default();
        coordinate_system(wo, &v1, &v2);
        *wi = spherical_direction(sinTheta, cos_theta, phi, v1, v2, -wo);
    ...
    pub fn is_medium_transition(&self) -> bool {
        if let Some(ref inside) = self.inside {
            if let Some(ref outside) = self.outside {
                let i = &*inside as *const _;
                let o = &*outside as *const _;
                i != o
            } else {
                true
            }
...
pub fn phase_hg(cos_theta: Float, g: Float) -> Float {
    ...
    return INV_4_PI * (1.0 - g * g) / (denom * denom.sqrt());
}

pub fn get_medium_scattering_properties(name: &str) -> Result<(Spectrum, Spectrum), ()> {
    for mss in SUBSURFACE_PARAMETER_TABLE {
        if mss.name == name {
            return Ok((Spectrum::from_rgb(mss.sigma_a), Spectrum::from_rgb(mss.sigma_prime_s)));
        }
    }
    Err()
}
...
const SUBSURFACE_PARAMETER_TABLE: [MeasuredSS] = [
	...
	MeasuredSS{
		...
		simgma_a: [0.0030, 0.0034, 0.046]
	},
	MeasuredSS{
		name: "Chicken1",
		sigma_prime_s: [0.15, 0.21, 0.38],
		simgma_a: [0.015, 0.077, 0.19]
	},
	MeasuredSS{
		name: "Chicken2",
		sigma_prime_s: [0.19, 0.25, 0.32],
		simgma_a: [0.018, 0.088, 0.20]
	},
	MeasuredSS{
		name: "Cream",
		sigma_prime_s: [7.38, 5.47, 3.15],
		simgma_a: [0.0002, 0.0028, 0.0163]
	},
	MeasuredSS{
		name: "Ketchup",
		sigma_prime_s: [0.18, 0.07, 0.03],
		simgma_a: [0.061, 0.97, 1.45]
	},
	MeasuredSS{
		name: "Marble",
		sigma_prime_s: [2.19, 2.62, 3.00],
		simgma_a: [0.0021, 0.0041, 0.0071]
	},
	MeasuredSS{
		name: "Potato",
		sigma_prime_s: [0.68, 0.70, 0.55],
		simgma_a: [0.0024, 0.0090, 0.12]
	},
	MeasuredSS{
		name: "Skimmilk",
		sigma_prime_s: [0.70, 1.22, 1.90],
		simgma_a: [0.0014, 0.0025, 0.0142]
	},
	MeasuredSS{
		name: "Skin1",
		sigma_prime_s: [0.74, 0.88, 1.01],
		simgma_a: [0.032, 0.17, 0.48]
	},
	MeasuredSS{
		name: "Skin2",
		sigma_prime_s: [1.09, 1.59, 1.79],
		simgma_a: [0.013, 0.070, 0.145]
	},
	MeasuredSS{
		name: "Spectralon",
		sigma_prime_s: [11.6, 20.4, 14.9],
		simgma_a: [0.00, 0.00, 0.00]
	},
	MeasuredSS{
		name: "Wholemilk",
		sigma_prime_s: [2.55, 3.21, 3.77],
		simgma_a: [0.0011, 0.0024, 0.014]
	},
